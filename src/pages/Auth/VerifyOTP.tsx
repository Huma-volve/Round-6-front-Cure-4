        ...
        <div className="lg:max-w-1/3 mt-20 lg:ml-55 px-5 lg:px-0">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              />
              {verifyOTP?.isError && <p style={{ color: 'red' }}>{verifyOTP?.error.response?.data?.message}</p>}
              <Button type="submit" className="text-white bg-[#145DB8] w-full cursor-pointer hover:bg-blue-700">{verifyOTP?.isLoading?<span className="animate-spin bg-white w-4 h-4 inline-block"></span>:'Send'}</Button>
            </form>
          </Form>
        </div>
      </div>
